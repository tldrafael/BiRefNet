...
from evaluation.evaluate import evaluate_evalset_by_cat
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        disable = True if to_be_distributed and get_rank() != 0 else False
        pbar_trainloader = tqdm(
            self.train_loader, desc=f"step {epoch}", disable=disable)
    ...
    def validate_model(self, epoch):
        ...
        for testset, data_loader_test in self.test_loaders.items():
            ...
            pred_dir = '.'
            method = 'tmp'
            inference(self.model, data_loader_test, pred_dir, method, testset, device=device)
            sadlog = evaluate_evalset_by_cat()
            print('SAD-LOG: {:.2f}'.format(sadlog))
            ...
            # print('MAE: {:.2f}, SAD-LOG: {:.2f}'.format(performance_dict['mae'], performance_dict['sad-log']))
